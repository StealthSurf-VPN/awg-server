...
uses its configured scalar/range value, with the `off` input alias rendered as
numeric `0`. `PresharedKey` is generated by
...
| `persistent_keepalive` | number or string | Client `[Peer]` value. 2.0 accepts only a literal scalar 0-65535 (omitted inherits 25; zero disables). 3.1 accepts the unsigned-16 grammar and compatibility alias below. It does not affect interface grouping. |
...
| `content_padding_addition` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `rekey_after_time` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `rekey_timeout` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `reject_after_time` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `keepalive_timeout` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `max_handshake_attempts` | number or string | 3.1 only: unsigned-16 scalar or range; `"off"` is accepted as an input alias for `0`. |
| `random_trailers` | string | 3.1 only: exactly `on` or `off`. |
| `disable_cookies` | string | 3.1 only: exactly `on` or `off`. |

The unsigned-16 grammar is ASCII decimal `N` or `N-M`. The compatibility input
alias `off` means `0`. It rejects whitespace, signed values, floats, exponent
notation, empty values, reversed ranges, and overflow. Before API output,
persistence, AWG rendering, and profile identity, `off` becomes numeric `0`
and an equal range `N-N` becomes numeric `N`; other ranges remain strings. A
2.0 request rejects every 3.1-only field and a syntactically ranged or `off`
`persistent_keepalive` before this canonicalization.
